...
class SetupWizard {
    ...
    public function setup_wizard() {
        ...
        // Check if using new design
        $use_new_design = apply_filters( 'erp_setup_use_new_design', true );

        wp_enqueue_style( 'jquery-ui', WPERP_ASSETS . '/vendor/jquery-ui/jquery-ui-1.9.1.custom.css' );
        
        if ( $use_new_design ) {
            wp_enqueue_style( 'erp-setup-new', WPERP_ASSETS . '/css/setup-new.css', [ 'dashicons' ], WPERP_VERSION );
        } else {
    ...
    public function setup_wizard_header() {
        $use_new_design = apply_filters( 'erp_setup_use_new_design', true );
        $body_class = $use_new_design ? 'erp-setup-new' : 'erp-setup';
        ?>
        <!DOCTYPE html>
        ...
        <body class="<?php echo esc_attr( $body_class ); ?> wp-core-ui">
            <?php if ( $use_new_design ) : ?>
            ...
            <h1 class="erp-logo">
                <?php if ( $use_new_design ) : ?>
                    ...
                    ?>
                <?php else : ?>
                    <a href="http://wperp.com/"><?php esc_html_e( 'WP ERP', 'erp' ); ?></a>
                <?php endif; ?>
            </h1>
        <?php
    }
    ...
    public function setup_wizard_steps() {
        ...
        if ( $use_new_design ) {
            $this->setup_wizard_steps_new();
        } else {
            $this->setup_wizard_steps_old();
        }
    }
    
    /**
     * Output the old steps design
     */
    private function setup_wizard_steps_old() {
    ...
    /**
     * Output the new steps design (progress bars)
     */
    private function setup_wizard_steps_new() {
        $all_steps = array_keys( $this->steps );
        $current_step_index = array_search( $this->step, $all_steps );
        $total_steps = count( $all_steps ) - 1; // Exclude introduction
        ...
        <div class="erp-progress-container">
            <div class="erp-progress-bars">
                <?php for ( $i = 1; $i <= $total_steps; $i++ ) : ?>
                    <div class="erp-progress-bar <?php echo $i <= $current_step_index ? 'active' : ''; ?>"></div>
                <?php endfor; ?>
            </div>
            <div class="erp-progress-counter">
                <?php echo esc_html( $current_step_index . '/' . $total_steps ); ?>
            </div>
            <?php if ( isset( $this->steps[ $this->step ]['name'] ) ) : ?>
                <div class="erp-step-label">
                    <?php echo esc_html( $current_step_index . '. ' . $this->steps[ $this->step ]['name'] ); ?>
                </div>
            <?php endif; ?>
        </div>
        <?php
    ...
    public function next_step_buttons() {
        ...
        if ( $use_new_design ) {
            ...
            <div class="erp-button-container">
                <button type="submit" class="erp-btn-continue" name="save_step">
    ...
    public function setup_step_basic() {
        ...
        if ( $use_new_design ) {
            $this->setup_step_basic_new( $company, $financial_month, $company_started );
        } else {
            $this->setup_step_basic_old( $company, $financial_month, $company_started, $business_type );
        }
    }
    
    /**
     * Basic setup - new design
     *
     * @return void
     */
    private function setup_step_basic_new( $company, $financial_month, $company_started ) {
        ?>
    ...
    /**
     * Basic setup - old design
     *
     * @return void
     */
    private function setup_step_basic_old( $company, $financial_month, $company_started, $business_type ) {
        ?>
