...
Accounting is at **81 cases** (`--list`: 44 in `accounting_money`, 37 in `e2e_tests`). The payment
...
**Three edit paths are now done — payment, expense and bill payment — and each failed differently.**
⚠️ A lead recorded here earlier cited the wrong function: `expenses.php:520-537` is
...
- **ERP-152 (Critical) — [erp-pro#969](https://github.com/wp-erp/erp-pro/issues/969)** — payment
  edit APPENDS a duplicate cash row with `trn_no = 0`. Trial balance Dr 2,700 vs Cr 1,800. **3/3.**
- **ERP-153 (Major) — [erp-pro#970](https://github.com/wp-erp/erp-pro/issues/970)** — the payment
  edit screen requests `GET /invoices/{payment voucher}` and renders blank. **6/6.**
- **ERP-154 (Critical) — [erp-pro#971](https://github.com/wp-erp/erp-pro/issues/971)** — expense edit
...
- **ERP-156 (Critical) — [erp-pro#973](https://github.com/wp-erp/erp-pro/issues/973)** — bill-payment edit fails four ways at once: the payment
  ...
  the line), and the vendor ledger is never touched while the debit/credit sides are reversed.
  Measured: Trial Balance `$2,550.00 / $2,800.00`, Cash up by the full payment, a $750 bill showing
  $1,000 due. **API-only** — no `:id/edit` route exists for pay-bills. **5/5.**

**Still unmeasured, in priority order:** the **pay-purchase** edit path (`pay-purchases.php`, same
shape as ERP-156), `erp_acct_convert_draft_to_expense()`, and
`erp_acct_update_data_into_people_trn_details()` (`transactions.php:1764`) — one `$wpdb->delete()`
with **no re-insert**, called on edit by invoices, bills and purchases (`invoices.php:570`,
`bills.php:361`, `purchases.php:502`), so editing any of those three may simply erase the customer's
ledger row.

New helpers this pass: `updatePayBillViaRest()` and a shared private `restRequest()` that
`updatePaymentViaRest()` now also uses. The four pay-bill guards live in `payments.spec.ts`, which
already owns Meridian Office Supplies and the bill-payment flow.
...
4. **Accounting — 81 cases**: all 29 screens, both money cycles end to end, and the reports asserted
