...
export class HrmPage {
        ...
        await Promise.all([
            this.page.waitForResponse(
                r =>
                    ...
                    (r.request().postData() ?? '').includes('erp-hr-new-dept'),
                { timeout: 30_000 },
            ),
        ...
        await Promise.all([
            this.page.waitForResponse(
                r =>
                    r.url().includes('admin-ajax.php') &&
                    r.request().method() === 'POST' &&
                    (r.request().postData() ?? '').includes('erp-hr-new-desig'),
                { timeout: 30_000 },
            ),
    ...
    async fillEmployeeBasics(emp: { first_name: string; last_name: string; email: string; hiring_date?: string }): Promise<void> {
    ...
    static async seed(api: ApiUtils): Promise<IdMap> {
        ...
        try {
            ...
            if (resp.ok()) {
                ...
                if (id) {
                    ...
                    try {
                        await dbUtils.dbQuery(`UPDATE wp_erp_hr_employees SET pay_type = 'monthly', status = 'active' WHERE user_id = ?`, [
                            Number(id),
                        ]);
                    } catch {
        ...
        try {
            ...
            const [resp, body] = await api.post(endPoints.leavePolicies, { data: { name: policy.name, days: policy.days } }, false);
            if (resp.ok()) {
                const id = idOf(body);
