...
function idOf(body: ResponseBody): string {
    const raw = (body as { voucher_no?: unknown })?.voucher_no ?? (body as { id?: unknown })?.id ?? '';
...
test.describe('Purchase Return REST — list (admin)', () => {
    ...
    test(
        'ACC-PR-02 list honors pagination query (?per_page=5&page=1) without 500',
    ...
    test(
        'ACC-PR-03 list page far past the end answers 200 + [] (offset past end)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'ACC-PR-05 list with a type= filter still answers < 500 (quirky WHERE, non-fatal)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'ACC-PR-06 list non-numeric per_page currently 500s (KNOWN BUG, documented)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Purchase Return REST — search invoice (admin)', () => {
    ...
    test(
        'ACC-PR-07 search-invoice on a REAL seeded purchase returns 200 + invoice object',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'ACC-PR-08 search-invoice on a non-existent id answers 200 with a synthetic object (null->array)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Purchase Return REST — create (admin)', () => {
    ...
    test(
        'ACC-PR-10 create a purchase return from a real seeded purchase (RESILIENT happy path)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            const seed = await seedPurchase();
            ...
            const invoiceDetailsId = firstLine && firstLine.invoice_details_id !== undefined ? Number(firstLine.invoice_details_id) : 0;
    ...
    test(
        'ACC-PR-11 created return is readable via GET /{id} (secondary single-read)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'ACC-PR-13 create against a NON-existent purchase voucher is tolerated (2xx OR 4xx-with-message)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            const payload = {
                ...
                voucher_no: 99999999,
                line_items: [{ invoice_details_id: 99999999, product_id: 999999, qty: 1, price: 10, tax: 0, discount: 0 }],
            };
    ...
    test(
        'ACC-PR-14 create with MISSING line_items is silently accepted as a 201 (KNOWN BUG, documented)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            expect(resp.status(), 'missing line_items is currently accepted as a clean 201 — KNOWN BUG, see bug-reports/BUGS.md').toBe(201);
            const obj = (body ?? {}) as Record<string, unknown>;
            const returnId = obj.id !== undefined && obj.id !== null ? String(obj.id) : '';
            if (returnId && /^\d+$/.test(returnId)) {
                createdReturnIds.push(returnId);
...
test.describe('Purchase Return REST — accounting manager (positive baseline)', () => {
    ...
    test(
        'ACC-PR-15 manager can reach the purchase-return list (erp_ac_create_expenses_voucher)',
    ...
    test(
        'ACC-PR-16 manager can reach search-invoice (erp_ac_view_sales_summary)',
        { tag: ['@pro', '@accounting', '@manager'] },
        async () => {
...
test.describe('Purchase Return REST — access control (no auth)', () => {
    ...
    test('ACC-PR-19 no-auth cannot create a purchase return', { tag: ['@pro', '@accounting', '@employee'] }, async () => {
        const [res] = await noAuthApi.post(PR_CREATE, { headers: { 'X-WP-Nonce': '' }, data: { vendor_id: 1, line_items: [] } }, false);
