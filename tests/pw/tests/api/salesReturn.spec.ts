...
test.describe('Sales Return REST — list (admin)', () => {
    ...
    test(
        'SR-EC-03 malformed (non-numeric) status is raw-interpolated into SQL — tolerate < 500',
...
test.describe('Sales Return REST — search-invoice (admin)', () => {
    ...
    test(
        'SR-HP-04 search-invoice for a real sales voucher returns it + line_items[]',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'SR-EC-04 search-invoice for a non-existent numeric voucher — null-deref risk tolerated',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Sales Return REST — single (admin)', () => {
    ...
    test(
        'SR-EC-06 single for a non-existent numeric voucher — null-deref risk tolerated',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Sales Return REST — create (admin)', () => {
    ...
    test(
        'SR-HP-05 create a sales return against a seeded invoice (201 OR documented 4xx)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            if (resp.status() !== 201) {
                ...
                test.info().annotations.push({
                    type: 'sales-return-create',
                    description: `create answered ${resp.status()} (4xx tolerated)`,
                });
                return;
            }
    ...
    test(
        'SR-HP-06 created sales return persists across the return tables — DB oracle',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            test.skip(!customerId || !invoiceVoucherNo, 'needs a seeded customer + sales invoice voucher_no');
            const payload = createPayload();
            ...
            const srRows = await dbUtils.dbQuery<{
            ...
            expect(String(sr?.comments ?? ''), 'comments overwritten with the voucher-no template (particulars not persisted)').toContain(
                returnVoucherNo,
            );
            ...
            const detailRows = await dbUtils.dbQuery<{
                invoice_details_id: number | string;
                qty: number | string;
                unit_price: number | string;
            }>(`SELECT invoice_details_id, qty, unit_price FROM ${T_SALES_RETURN_DETAILS} WHERE trn_no = ?`, [returnVoucherNo]);
            ...
            expect(Number(detailRows[0]?.unit_price ?? -1), 'detail unit_price matches the submitted line').toBeCloseTo(
                submitted?.unit_price ?? 0,
                2,
            );
        },
    );
    ...
    test(
        'SR-EC-08 create skips line_items with qty <= 0 (totals derived from positive lines only)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            expect(Number((body as { amount?: unknown })?.amount ?? -1), 'amount counts only the positive-qty line').toBeCloseTo(
                line.unit_price,
                2,
            );
        },
    ...
    test(
        'SR-EC-09 create with empty line_items derives zero totals (201 OR documented 4xx)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            test.skip(!customerId || !invoiceVoucherNo, 'needs a seeded customer + sales invoice voucher_no');
    ...
    test(
        'SR-NC-01 create with a missing customer_id is silently accepted as a customer-less return (KNOWN BUG)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            test.skip(!invoiceVoucherNo, 'needs a seeded sales invoice voucher_no');
    ...
    test(
        'SR-NC-02 create against a non-existent source voucher_no is NOT a clean success',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Sales Return REST — accounting manager (positive baseline)', () => {
    ...
    test(
        'SR-AC-01 manager can reach the sales-return list (erp_ac_view_sales_summary)',
    ...
    test(
        'SR-AC-03 manager is NOT auth-refused at the create route (read cap gates create — finding)',
        { tag: ['@pro', '@accounting', '@manager'] },
        async () => {
