...
test.describe('Reimbursement REST — employee requests (admin)', () => {
    ...
    test('REIMB-HP-03 list filtered by people_id', { tag: ['@pro', '@hrm', '@admin'] }, async () => {
        ...
        if (resp.status() === 200) {
            ...
            if (Array.isArray(body) && body.length > 0) {
                const allMatch = body.every((r: { people_id?: number | string }) => String(r?.people_id ?? '1') === '1');
    ...
    test('REIMB-HP-05 create reimbursement request + DB-assert request & detail rows', { tag: ['@pro', '@hrm', '@admin'] }, async () => {
        ...
        const detail = await dbUtils.dbQuery<{ c: number }>(`SELECT COUNT(*) AS c FROM ${REIMB_REQUEST_DETAILS} WHERE request_id = ?`, [
            row.id,
        ]);
        expect(Number(detail[0]?.c ?? 0), 'one detail row per line item').toBe((payload.line_items as unknown[]).length);
    });
    ...
    test('REIMB-HP-06 get single reimbursement request by id (round-trip after create)', { tag: ['@pro', '@hrm', '@admin'] }, async () => {
        ...
        const rows = await dbUtils.dbQuery<{ id: number }>(`SELECT id FROM ${REIMB_REQUESTS} WHERE reference = ? LIMIT 1`, [
            payload.reference,
        ]);
    ...
    test(
        'REIMB-EC-01 create WITHOUT line_items — KNOWN BUG: silently accepted as a clean 201',
        { tag: ['@pro', '@hrm', '@admin'] },
        async () => {
            ...
            if (ref) {
                const leaked = await dbUtils.dbQuery<{ id: number }>(`SELECT id FROM ${REIMB_REQUESTS} WHERE reference = ?`, [ref]);
    ...
    test(
        'REIMB-EC-03 get numeric-but-nonexistent id — documented <500 (reset() on empty)',
        { tag: ['@pro', '@hrm', '@admin'] },
        async () => {
    ...
    test(
        'REIMB-EC-04 attachments route is registered (JSON smoke, not 404-route-missing)',
        { tag: ['@pro', '@hrm', '@admin'] },
        async () => {
...
test.describe('Reimbursement REST — manager chart routes (manager)', () => {
    ...
    test(
        'REIMB-HP-10 manager chart-requests (erp_ac_view_sales_summary) answers, not auth-denied',
        { tag: ['@pro', '@hrm', '@manager'] },
        async () => {
...
test.describe('Reimbursement REST — people transactions (admin)', () => {
    ...
    test(
        'REIMB-HP-21 people transactions list accepts pagination params (array, no 5xx)',
        { tag: ['@pro', '@hrm', '@admin'] },
        async () => {
    ...
    test(
        'REIMB-EC-22 create people transaction — resilient (best-effort, may 5xx on missing deps)',
        { tag: ['@pro', '@hrm', '@admin'] },
        async () => {
