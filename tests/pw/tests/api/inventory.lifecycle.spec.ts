...
async function resolveInvoiceVoucherNo(id: string | number): Promise<string> {
    try {
        const rows = await dbUtils.dbQuery<{ voucher_no: number | string }>(`SELECT voucher_no FROM ${INVOICES} WHERE id = ?`, [id]);
...
test.describe('Inventory lifecycle — CSV products import surface (admin)', () => {
    ...
    test(
        'INVLC-HP-01 validate accepts a well-formed product CSV → 200 + {data, update:[], total:N}',
    ...
    test(
        'INVLC-BUG-01 KNOWN BUG: import returns 201/body=total but inserts ZERO products',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            try {
                before = await dbUtils.dbQuery<{ c: number }>(`SELECT COUNT(*) AS c FROM ${PRODUCTS} WHERE name IN (${inClause})`, names);
            } catch {
            ...
            const after = await dbUtils.dbQuery<{ c: number }>(`SELECT COUNT(*) AS c FROM ${PRODUCTS} WHERE name IN (${inClause})`, names);
            expect(
                Number(after[0]?.c ?? -1),
                'KNOWN BUG: import inserted ZERO products despite the 201 (VALUES %s double-quotes the tuple-list)',
            ).toBe(0);
        },
    );

    test(
        'INVLC-NC-01 validate with an empty category_id column surfaces the import-error envelope',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            if (status === 500) {
                test.info().annotations.push({
                    type: 'bug-candidate',
                    description:
                        'CSV validate returns a bare WP_Error (no status) → HTTP 500 for a plain validation failure (expected a 4xx)',
                });
            }
        },
    );
    ...
    test(
        'INVLC-NC-02 validate rejects a non-CSV (txt) file with invalid-file-type',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            const [status, body] = await postValidate(csvMultipart('just some text, not a real csv\n', 'text/plain', 'notcsv.txt'));
    ...
    test(
        'INVLC-EC-01 validate of a duplicate product name surfaces a unique:name validation error',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
...
test.describe('Inventory lifecycle — stock movement (admin)', () => {
    ...
    test(
        'INVLC-HP-03 full lifecycle: a purchase + sale of an inventory product propagate to the ledger',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            // ── 3. PRODUCT (type 1 = Inventory; REQUIRED for the stock hook to fire).
            lifeProductName = `LifeProd_${RUN}`;
            const [, productBody] = await api.post(
                endPoints.acctProducts,
                {
                    data: {
            ...
            const [, vendorBody] = await api.post(
                endPoints.acctVendors,
                {
            ...
            const [, purchaseBody] = await api.post(
                endPoints.acctPurchases,
                {
                    data: {
            ...
            const [, customerBody] = await api.post(
                endPoints.acctCustomers,
                {
            ...
            if (stockResp.status() === 200) {
                expect(overviewNum(stockAfterBody, 'stock_in') - stockInBefore, 'stock-overview stock_in rose by the purchase qty').toBe(
                    PURCHASE_QTY,
                );
                expect(overviewNum(stockAfterBody, 'stock_out') - stockOutBefore, 'stock-overview stock_out rose by the sale qty').toBe(
                    SALE_QTY,
                );
            }
            ...
            if (trnResp.status() === 200) {
                expect(
                    overviewNum(trnAfterBody, 'sales') - salesBefore,
                    'transactions-overview sales rose by the invoice amount',
                ).toBeCloseTo(SALE_QTY * SALE_UNIT, 2);
                expect(
                    overviewNum(trnAfterBody, 'purchase') - purchaseBefore,
                    'transactions-overview purchase rose by the purchase amount',
                ).toBeCloseTo(PURCHASE_QTY * PURCHASE_UNIT, 2);
            }
        },
    );

    test(
        "INVLC-HP-04 reports/purchase shows this run's purchase voucher with qty + price",
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        "INVLC-HP-05 reports/sales shows this run's invoice voucher with qty + price",
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
    ...
    test(
        'INVLC-HP-06 reports/item-summary shows correct no_purchase (no_sale is the documented bug)',
        { tag: ['@pro', '@accounting', '@admin'] },
        async () => {
            ...
            if (row) {
                ...
                expect(Number(row.no_sale ?? -1), 'KNOWN BUG: no_sale duplicates no_purchase (sum(stock_in)) instead of the sale qty').toBe(
                    PURCHASE_QTY,
                );
                ...
                test.info().annotations.push({
                    type: 'bug-candidate',
                    description:
                        'reports/item-summary no_sale = sum(stock_in) (reports.php:163-164) → equals no_purchase, not the sale qty',
                });
            }
        },
    );
...
test.describe('Inventory lifecycle — accounting manager (positive baseline)', () => {
    ...
    test(
        'INVLC-AC-01 manager is NOT auth-refused at the CSV validate route (erp_ac_manager)',
