...
#### CORE-T3-001 — `admin.php?page=erp` is closed to roles that do not own it

- **Surface:** `admin.php?page=erp`
- **Tags:** @tier3 @core @authz
- **Steps:**
  1. For each of erp_hr_manager, erp_crm_manager, erp_crm_agent, erp_ac_manager, erp_recruiter, employee, a logged-out visitor, open `admin.php?page=erp` directly by URL.
