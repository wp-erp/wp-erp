<template>
    <div class="sales-tax-report">
        <h2>{{ __('Purchase Tax Report Agency Based', 'erp') }}</h2>

        <form action="" method="" @submit.prevent="getSalesTaxReport" class="query-options no-print">
            <div class="with-multiselect">
                <multi-select v-model="taxAgency" :options="taxAgencies"/>
            </div>
        ...
        <ul class="report-header" v-if="null !== taxAgency">
            <li><strong>{{ __('Agency Name', 'erp') }}:</strong> <em>{{ taxAgency.name }}</em></li>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getSalesTaxReport() {
                if (this.taxAgency === null) return;

                this.$store.dispatch('spinner/setSpinner', true);

                this.rows = [];

                HTTP.get('/tax-reports/agency-wise-purchase', {
                    params: {
