<template>
    <div class="sales-tax-report">
        <h2>{{ __('Purchase Tax Report Category Based', 'erp') }}</h2>
        ...
        <ul class="report-header" v-if="null !== taxCategory">
            <li><strong>{{ __('Category Name', 'erp') }}:</strong> <em>{{ taxCategory.name }}</em></li>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getSalesTaxReport() {
                if (this.taxCategory === null) return;

                this.$store.dispatch('spinner/setSpinner', true);

                this.rows = [];

                HTTP.get('/tax-reports/category-wise-purchase', {
                    params: {
