...
function erp_ac_dashboard_left_column() {
    erp_admin_dash_metabox( __( 'Income & Expenses', 'accounting' ), 'erp_ac_dashboard_income_expense' );
    erp_admin_dash_metabox( __( 'Cash & Bank Balance', 'accounting' ), 'erp_ac_dashboard_banks', 'bank-balance' );

    erp_admin_dash_metabox( __( 'Invoice payable to you', 'accounting' ), 'erp_ac_dashboard_invoice_payable' );
}
...
function erp_ac_dashboard_right_column() {
    erp_admin_dash_metabox( __( 'Business Expense', 'accounting' ), 'erp_ac_dashboard_expense_chart' );
    erp_admin_dash_metabox( __( 'Net Income', 'accounting' ), 'erp_ac_dashboard_net_income', 'bank-balance' );
    erp_admin_dash_metabox( __( 'Bills you need to pay', 'accounting' ), 'erp_ac_dashboard_bills_payable' );
...
function erp_ac_dashboard_banks() {
    ...
    <ul>
    	<?php foreach ( $bank_journals as $id => $journal ) {
    		...
    		$bank_url = admin_url( 'admin.php?page=erp-accounting-charts&action=view&id=' . $journal['id'] );
    		?>
    		<li><span class="account-title"><a href="<?php echo $bank_url; ?>"><?php echo $journal['name']; ?></a></span> <span class="price"><a href="#"><?php echo erp_ac_get_price( $journal['total_journal'] ); ?></a></span></li>
    		<?php
    	}
    	?>

        <li class="total">
            <span class="account-title"><?php _e( 'Total', 'accounting' ); ?></span> <span class="price"><a href="#"><?php echo erp_ac_get_price( $total ); ?></a></span>
        </li>
    </ul>
    <?php
}
...
function erp_ac_dashboard_net_income() {
	...
	$net_income = abs( $income - $expense );
	$symbole = erp_ac_get_currency_symbol();
	?>
    <ul>
        <li><span class="account-title"><?php _e( 'Income', 'accounting' ); ?></span> <span class="price"><a href="#"><?php echo erp_ac_get_price( $income ); ?></a></span></li>
        <li><span class="account-title"><?php _e( 'Expense', 'accounting' ); ?></span> <span class="price"><a href="#"><?php echo erp_ac_get_price( $expense ); ?></a></span></li>
        <li class="total">
            <span class="account-title"><?php _e( 'Net Income', 'accounting' ); ?></span> <span class="price"><a href="#"><?php echo erp_ac_get_price( $net_income ); ?></a></span>
        </li>
    </ul>
    <?php
}
	...
	foreach ($incomes as $key => $income ) {
		...
		foreach ( $income as $key => $journal_val ) {
			...
			foreach ( $journal_val['journals'] as $key => $journal ) {
				...
				$total = $total + abs( $journal['debit'] - $journal['credit'] );
			}
		}
	...
	foreach ( $expenses as $key => $expense ) {
		...
		foreach ( $expense as $key => $journal_val ) {
			...
			foreach ( $journal_val['journals'] as $key => $journal ) {
				...
				$expense_data[$journal['ledger_id']][] = abs( $journal['debit'] - $journal['credit'] );
			}
		}
	}
	$ledger_data = [];
