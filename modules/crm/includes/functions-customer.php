...
function erp_crm_render_save_replies( $template_id, $contact_id ) {
    ...
    foreach ( $shortcodes as $shortcode => $shortcode_val ) {
        if ( $shortcode_val['is_meta'] ) {
            ...
            if( property_exists( $contactsInfo, $shortcode_val['key'] ) ) {
                if ($shortcode == '%country%') {
                    $data[] = erp_get_country_name($contactsInfo->{$shortcode_val['key']});
                } elseif ($shortcode == '%state%') {
                    $data[] = erp_get_state_name($contactsInfo->country, $contactsInfo->{$shortcode_val['key']});
                } else {
                    $data[] = $contactsInfo->{$shortcode_val['key']};
                }
            }
        }
    }
