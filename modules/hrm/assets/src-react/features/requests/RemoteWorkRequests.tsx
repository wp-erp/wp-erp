            ...
            .catch((e) =>
                toast.error(
                    (e as ApiError)?.message ||
                        __("Could not load remote work requests.", "erp"),
                ),
            )
...
function NewRemoteWorkDialog({
    ...
    const [from, setFrom] = useState(todaySiteYmd());
    const [to, setTo] = useState(todaySiteYmd());
    const [busy, setBusy] = useState(false);
    ...
    function submit(): void {
        ...
        request(restPath("v2", BASE), {
            ...
            .then(() => {
                toast.success(__("Remote work request submitted.", "erp"));
                onSaved();
            })
            .catch((e: ApiError) =>
            ...
            <DialogContent className="max-h-[90vh] gap-4 overflow-y-auto rounded-[10px] p-6 sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="m-0 mb-4 text-2xl font-bold leading-tight tracking-tight text-foreground">
                        {__("New Remote Work Request", "erp")}
                    </DialogTitle>
                    <DialogDescription>
                        {__(
                            "File a remote work request on an employee's behalf.",
                            "erp",
                        )}
                    </DialogDescription>
                </DialogHeader>
